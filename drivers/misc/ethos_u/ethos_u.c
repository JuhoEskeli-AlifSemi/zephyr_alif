 ...
 * On this SoC the power domain may remove power from the NPU while suspended,
 * so resume performs a full re-initialisation and suspend enables NPU clock
 * and power gating for STOP mode before tearing the driver back down to a
 * clean state.
 *
...
static int ethosu_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		/*
		 * The power domain may have removed power from the NPU while
		 * suspended, clearing all register state. Re-initialise the
		 * hardware and re-register the driver handle. ethosu_zephyr_init()
		 * also re-enables the NPU IRQ via config->irq_config().
		 */
