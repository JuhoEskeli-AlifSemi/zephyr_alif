...
#define EVT_RING_SIZE		16  /* TRBs in event ring */
...
/* Data buffer for USB control transfers (config descriptor can be up to 256 bytes) */
static uint8_t xfer_data_buf[256] __aligned(64) UHC_DMA_SECTION;
			       ...
			       uint8_t slot_id, uint8_t port_speed)
{
	...
	uint16_t mps = (port_speed == XHCI_SPEED_HS) ? 64 :
		       (port_speed == XHCI_SPEED_LS) ? 8 : 8;
				      ...
				      struct usb_device_descriptor *desc)
{
	...
	/*
	 * Setup TRB: GET_DESCRIPTOR(Device, index 0) for 18 bytes.
	 *
	...
	/* Data TRB: point to data buffer, length=18, direction=IN */
			      ...
			      struct usb_device_descriptor *desc)
{
	...
	uint8_t port_speed = XHCI_PORTSC_SPEED(xhci_portsc_read(cfg));

	LOG_INF("Port speed after reset: %u", port_speed);
	...
	ret = xhci_get_device_descriptor(cfg, slot_id, desc);
	if (ret) {
		return ret;
	}
